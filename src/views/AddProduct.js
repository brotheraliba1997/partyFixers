...
function User() {
  ...
  // const handleProductImage = async (data) => {
  ...
  const submit = async (e) => {
    e.preventDefault();
    // product3.filter((data) => {

    //   if (uid == data.id) {
    //     let answer = confirm("ID Already Available !!!! Are You Sure & Wanted To Replace This ID ....!!!!!!");
    //     if (answer == false) {
    //       // await deleteDoc(doc(db, "product", id));
    //       e.preventDefault()
    //     } else {
    //       if (imageUrl !== "") {
    //         setDoc(doc(db, "product", uid), {
    ...
    if (imageUrl !== "") {
      ...
      // await batch.set(doc(db, "product", uid), {
  ...
  React.useEffect(() => {
    ...
    const unsub2 = onSnapshot(collectionProduct, (snapshot) => {
      const data = snapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id }));
      setProduct2(data);
      // setProduct4(data);
    });
  ...
  React.useEffect(() => {
    ...
    const unsub3 = onSnapshot(collectionProduct, (snapshot) => {
      const data = snapshot.docs.map((doc) => ({ ...doc.data(), id: doc.id }));
      setProduct3(data);
    });
  ...
  const SelectCategory = () => {
    setFilterCategory(product3.filter((data, i) => { for (var i = 0; i < data.category.length; i++) { if (data.category[i] === category2) { return data } } }));
  };
  ...
  const handleDiscountProduct = (e, rate) => {
    // console.log(">>>>>>r", rate)
    // console.log(">>>>>>re", e.target.value)
    // var discountValue = rate * ((100 - e.target.value) / 100)
    // var discountValue = rate / (e.target.value / 100)
    var discountValueProduct = ((rate - e.target.value) / rate) * 100

    setDiscountPer(discountValueProduct.toFixed())
  ...
  const handleUID = async (e) => {
    // console.log("id>>>", product3.filter(data => { if (data.id === e.target.value) { alert("id availble") }else{setUid(e.target.value)} }))
    // if (product3) {
    product3.length ?
      product3.filter(data => {
  ...
  // React.useEffect(() => {
  //   const collectionProduct = collection(db, "testing");
                      ...
                      <Col className="px-1" md="4">
                        <Form.Group>
                          ...
                          <Form.Control
                            ...
                            maxLength="2"
                            type="number"
                            onChange={(e) => handleDiscountProduct(e, rate)}
              ...
              <CardContent>
                <div className="artypo">
                  ...
                  {/* <span>
                    ...
                    //     type="checkbox" name={data.category} id={data.category} value={data.category} />
                    //   <label for={data.category} id={data.category}>{data.category}</label>
                    // </span> */}
                  <List sx={{ width: '100%', maxWidth: 360, bgcolor: 'background.paper' }}>
                    {product2.map((value) => {
      ...
      {/* <Container fluid>
        <Row>
          <Col md="8">
            <Card>
              <Card.Header>
                <Card.Title as="h4">Add Sale To All Products</Card.Title>
              </Card.Header>
              <Card.Body>
                <Form>
                  <Row>
